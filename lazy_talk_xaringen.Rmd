---
title: "R is Lazy... Yay!"
subtitle: "Lazy loading, evaluation, and queries"
...
```
# Lazy Evaluation and Execution Environment 
...
> |Symbol|Explanation
> |----|-------------
> | ![](images/green-check.png)| A single green check indicates that some rows are returned. <br>
> | ![](images/green-check.png) ![](images/green-check.png)| Two green checks indicate that all the rows are returned.
> | ![](images/red-x.png) |The red X indicates that no rows are returned.
>
...
> [`Qc <- Q %>% count(saleslastyear, sort = TRUE)`](#lazy_q_build) | ![](images/red-x.png) **Extends** the lazy query object
> 
> 
...
![](images/green-check.png) Because the following statement implies a `print()` function at the end, we can run it repeatedly, adding dplyr expressions, and only get 10 rows back.  Every time we add a dplyr expression to a chain, R will rewrite the SQL code.  For example:
...
![](images/green-check.png) When all the accumulated `dplyr` verbs are executed, they are submitted to the dbms and the number of rows that are returned follow the same rules as discussed above.
...
![](images/green-check.png) It just so happens that PostgreSQL has a `date` function that does the same thing as the `date` function in the `lubridate` package.  In the following code the `date` function is executed by PostreSQL.
```{r postgresql homonym}
# sales_person_table %>% mutate(birthdate = date(birthdate))
```
![](images/green-check.png) ![](images/green-check.png) If we specify that we want to use the `lubridate` version (or any number of other R functions) they are passed to the dbms unless we explicitly tell `dplyr` to stop translating and bring the results back to the R environment for local processing.
